...
import { myListings, requestsFor, USERS, usePass } from '@/pass/store';
    ...
    <Screen>
      ...
      {list.length === 0 ? (
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 40 }}>
          ...
          <Text style={[t.h3, { marginTop: 16 }]}>No listings yet</Text>
          <Text style={[t.small, { marginTop: 8, textAlign: 'center' }]}>Post something you no longer need and choose who it goes to.</Text>
          <Btn icon="add" label="Post an item" onPress={newListing} style={{ marginTop: 18, paddingVertical: 12, paddingHorizontal: 22 }} textStyle={{ fontSize: 14 }} />
        </View>
      ) : (
              ...
              <View key={item.id} style={{ backgroundColor: C.surface, borderRadius: radius.xl, borderCurve: 'continuous', padding: 15, ...shadow(10, 26, 0.4) }}>
                <Pressable onPress={() => openItem(item.id)} style={({ pressed }) => ({ flexDirection: 'row', gap: 12, alignItems: 'center', opacity: pressed ? 0.7 : 1 })}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={{ fontSize: 12, color: C.muted, marginTop: 2 }} numberOfLines={1}>{item.cat} · {item.cond}</Text>
                  </View>
                  <View style={{ backgroundColor: item.taken ? C.bg : '#E4F0E9', borderRadius: radius.pill, paddingVertical: 5, paddingHorizontal: 11 }}>
                    <Text style={{ fontSize: 11, fontWeight: '700', color: item.taken ? C.muted : C.free }} numberOfLines={1}>
                      {item.taken ? `Given to ${item.takenBy ? USERS[item.takenBy].name : 'someone'}` : 'Live'}
                    </Text>
                  </View>
                </Pressable>
                ...
                {!item.taken && requestsFor(s, item.id).length > 0 ? (
                  <View style={{ marginTop: 14, gap: 8 }}>
                    <Text style={{ fontSize: 11, fontWeight: '700', color: C.muted, letterSpacing: 0.3 }}>REQUESTS</Text>
                      ...
                      <View key={request.id} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, borderWidth: 1, borderColor: C.line, borderRadius: 14, padding: 10 }}>
                        ...
                        {request.status === 'pending' ? (
                          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                            ...
                            <Btn label="Accept" onPress={() => acceptRequest(request.id)} style={{ paddingVertical: 9, paddingHorizontal: 16 }} textStyle={{ fontSize: 13 }} />
                          </View>
                        ) : (
                ...
                {!item.taken ? (
                  <View style={{ flexDirection: 'row', gap: 10, marginTop: 14 }}>
                    <Btn icon="pencil" label="Edit" variant="outline" onPress={() => edit(item.id)} style={{ flex: 1, paddingVertical: 11 }} textStyle={{ fontSize: 14 }} />
                    <Btn icon="check" label="Mark as Taken" onPress={() => openTakenPicker(item.id)} style={{ flex: 1, paddingVertical: 11 }} textStyle={{ fontSize: 14 }} />
                  ...
                  <Btn icon="trash" label="Delete listing" variant="outline" onPress={() => remove(item)} block style={{ marginTop: 14, paddingVertical: 11, borderColor: C.dangerBorder }} textStyle={{ fontSize: 14, color: C.dangerInk }} />
                )}
              </View>
            ))}
          </View>
      ...
      <Modal visible={s.takenPickerId !== null} transparent animationType="slide" onRequestClose={() => patch({ takenPickerId: null })}>
        ...
        <View style={{ backgroundColor: C.surface, borderTopLeftRadius: 26, borderTopRightRadius: 26, padding: 22, paddingTop: 8 }}>
          ...
          <Text style={t.h3}>Who picked it up?</Text>

          {picker.length === 0 ? (
            <Text style={[t.small, { marginTop: 14 }]}>No one has requested this item yet.</Text>
          ) : (
            <View style={{ gap: 10, marginTop: 16 }}>
              {picker.map(({ request, user }) => (
                <View key={request.id} style={{ flexDirection: 'row', gap: 12, alignItems: 'flex-start', borderWidth: 1, borderColor: C.line, borderRadius: 15, padding: 12 }}>
                  ...
                  <Btn label="Choose" onPress={() => confirmTaken(s.takenPickerId!, request.fromUserId)} style={{ paddingVertical: 9, paddingHorizontal: 16 }} textStyle={{ fontSize: 12.5 }} />
                </View>
              ))}
            </View>
          )}

          <Btn label="Cancel" variant="ghost" onPress={() => patch({ takenPickerId: null })} block style={{ marginTop: 18 }} />
        </View>
      </Modal>
    </Screen>
  );
