...
- **Outbox is uid-tagged + logout is sync-gated (no data loss on account switch):** every optimistic write in the store must be wrapped in `track(...)` (from `outbox.ts`) — including the fire-and-forget `(async () => …)()` upload-then-upsert blocks. `logout()` sets `state.syncing` (shows the blocking `SyncOverlay`), then `await drainOutbox()` which awaits in-flight writers + flushes and returns **true only when nothing remains queued/in-flight** (no timeout). If it returns false (offline / a write couldn't be confirmed) logout **refuses** — it returns `{ ok:false }`, keeps the session, and the caller shows the `sync.cantLogout*` alert. Only on full sync does it `signOut()` (guarded — the offline revoke may throw). Queued ops carry the author's `uid`; `flushOutbox` never replays another user's op (RLS would deny it forever and could corrupt the other account). When adding a new write, route it through `track()`. Also: in `submitPost`, persist the listing **row first** (photos stripped to remote-only), then upload + second upsert — so a quick logout can't strand the row behind a slow photo upload. `syncing` is a transient flag — keep it out of the persist whitelist.
